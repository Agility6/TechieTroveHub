...
import com.TechieTroveHub.service.UserCenterService;
...
public class UserCenterApi {
    ...
    public JsonResponse<PageResult<Video>> pageListUserVideos(@RequestParam Integer size, @RequestParam Integer no, String area) {
        Long userId = userSupport.getCurrentUserId();
        PageResult<Video> result = userCenterService.pageListUserVideos(size, no, area, userId);
        return new JsonResponse<>(result);
    }
    ...
    public JsonResponse<Map<String, Object>> pageListUserCenterCollections(@RequestParam Integer size, @RequestParam Integer no, Long groupId) {
        Long userId = userSupport.getCurrentUserId();
    ...
    public JsonResponse<PageResult<UserFollowing>> pageListUserCenterFollowings(@RequestParam Integer size, @RequestParam Integer no, Long groupId) {
        Long userId = userSupport.getCurrentUserId();
        PageResult<UserFollowing> list = userCenterService.pageListUserCenterFollowings(userId, size, no, groupId);
     ...
     * 分页查询用户粉丝
     */
    public JsonResponse<PageResult<UserFollowing>> pageListUserFans(@RequestParam Integer size, @RequestParam Integer no) {
        Long userId = userSupport.getCurrentUserId();
